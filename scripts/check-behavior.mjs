...
const checks = [
  ...
  ['お気に入りには現行指定Skillを登録する', hallOfFameSkills.length === 11 && hallOfFameSkills.every((record) => record.kind === 'skill') && ['strict-recheck-and-refine', 'todo-add', 'elegant-prompt', 'communication-approval', 'shiryo-slides', 'shiryo-slides-hub', 'visual-management-board', 'cloudflare-web-publish', 'minutes-full-flow', 'pj-board', 'slack-to-reply-draft'].every((name) => hallOfFameSkills.some((record) => record.name === name))],
  ...
  ,['議事録まるっと・PJボード・Slack TO返信案はお気に入り・起動語・両環境実動未検証で表示する', [['minutes-full-flow','議事録まるっと','議事録登録して'], ['pj-board','PJボード','PJボード更新して'], ['slack-to-reply-draft','Slack TO返信案','Slack TO返信案']].every(([id, jp, trigger]) => { const record=all.find((item) => item.id === `user.${id}`); return record?.category === 'hall-of-fame' && record.japaneseName === jp && record.triggers?.includes(trigger) && ['codex', 'claude'].every((environment) => record.environments[environment].availability === 'installed_unverified' && /実動テストは未実施/.test(record.environments[environment].verificationNote || '')); })]
