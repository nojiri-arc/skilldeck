...
const checks = [
  ...
  ['お気に入りには現行指定Skillを登録する', hallOfFameSkills.length === 8 && hallOfFameSkills.every((record) => record.kind === 'skill') && ['strict-recheck-and-refine', 'todo-add', 'elegant-prompt', 'strategy-execution-orchestrator', 'initiative-progress-manager', 'discord-ai-relay', 'communication-approval', 'shiryo-slides'].every((name) => hallOfFameSkills.some((record) => record.name === name))],
  ...
  ,['共通Skillの英語名・日本語名・依頼例・起動語が正しい', (() => { const strategy=all.find((record) => record.id === 'user.strategy-execution-orchestrator'); const progress=all.find((record) => record.id === 'user.initiative-progress-manager'); return strategy?.name === 'strategy-execution-orchestrator' && strategy.japaneseName === '戦略→実行 統括' && strategy.example === '施策開始！' && strategy.triggers?.includes('$strategy-execution-orchestrator') && progress?.name === 'initiative-progress-manager' && progress.japaneseName === '施策進行マネージャー' && progress.example === '施策進行！' && ['施策のスケ調整お願い', '$initiative-progress-manager'].every((trigger) => progress.triggers?.includes(trigger)); })()]
