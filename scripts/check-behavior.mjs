...
const checks = [
  ...
  ['お気に入りには現行指定Skillを登録する', hallOfFameSkills.length === 15 && hallOfFameSkills.every((record) => record.kind === 'skill') && ['strict-recheck-and-refine', 'todo-add', 'elegant-prompt', 'communication-approval', 'shiryo-slides', 'shiryo-slides-hub', 'visual-management-board', 'cloudflare-web-publish', 'minutes-full-flow', 'pj-board', 'slack-to-reply-draft', 'link-collection', 'independent-review-session', 'ads-operations', 'permission-allowlist-sync'].every((name) => hallOfFameSkills.some((record) => record.name === name))],
  ...
  ,['Obsidian記録は指定カテゴリ・起動語・両環境の実行未検証状態で表示する', (() => { const record=all.find((item) => item.id === 'user.obsidian-record'); return record?.name === 'obsidian-record' && record.japaneseName === 'Obsidian記録' && record.category === 'meeting-writing-translation' && record.projectTags.includes('AI運用') && record.example === 'Obsidianに記録して' && ['Obsidianに記録して', 'メモしといて', '方針更新して', '$obsidian-record'].every((trigger) => record.triggers?.includes(trigger)) && /minutes-full-flow/.test(record.description) && ['codex', 'claude'].every((environment) => record.environments[environment].availability === 'installed_unverified' && record.environments[environment].evidenceType === 'file_hash_verified' && /実動テストは未実施/.test(record.environments[environment].verificationNote || '')); })()]
  ,['shiryo-slidesはお気に入り・外部提供・両環境実動未検証で表示する', (() => { const record=all.find((item) => item.id === 'user.shiryo-slides'); return record?.category === 'hall-of-fame' && record.provider.type === 'external' && record.provider.name === 'code4biz/shiryo-slides-kit' && record.example === 'この議事録から提案資料を作って' && ['資料を作って', '提案書を作って', 'スライドにして', '議事録から資料に', '$shiryo-slides'].every((trigger) => record.triggers?.includes(trigger)) && /ページ形式のHTMLスライド/.test(record.description) && /PPTXやGoogle Slides/.test(record.description) && ['codex', 'claude'].every((environment) => /^sha256:[0-9a-f]{64}$/.test(record.environments[environment].contentHash || '') && record.environments[environment].availability === 'installed_unverified' && record.environments[environment].evidenceType === 'file_hash_verified' && /実動テストは未実施/.test(record.environments[environment].verificationNote || '')); })()]
  ...
  ,['議事録まるっと・PJボード・Slack TO返信案・リンク集・使い捨て検品セッションはお気に入り・起動語・両環境実動未検証で表示する', [['minutes-full-flow','議事録まるっと','議事録登録して'], ['pj-board','PJボード','PJボード更新して'], ['slack-to-reply-draft','Slack TO返信案','Slack TO返信案'], ['link-collection','リンク集','リンク追加！'], ['independent-review-session','使い捨て検品セッション','検品！']].every(([id, jp, trigger]) => { const record=all.find((item) => item.id === `user.${id}`); return record?.category === 'hall-of-fame' && record.japaneseName === jp && record.triggers?.includes(trigger) && ['codex', 'claude'].every((environment) => record.environments[environment].availability === 'installed_unverified' && /実動テストは未実施/.test(record.environments[environment].verificationNote || '')); })]
