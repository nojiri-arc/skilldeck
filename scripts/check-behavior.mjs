...
const checks = [
  ...
  ['殿堂入りには現行指定Skillを登録する', hallOfFameSkills.length === 6 && hallOfFameSkills.every((record) => record.kind === 'skill') && ['strict-recheck-and-refine', 'todo-add', 'エレガント・プロンプト', '戦略→実行 統括', '施策進行マネージャー', 'discord-ai-relay'].every((name) => hallOfFameSkills.some((record) => record.name === name))],
