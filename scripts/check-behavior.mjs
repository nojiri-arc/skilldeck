...
const recommendedSkills = all.filter((record) => record.category === 'recommended');
...
const checks = [
  ...
  ['全5カテゴリに表示対象がある', data.categories.length === 5 && Object.values(categoryCounts).every((count) => count > 0)],
  ['第1カテゴリはおすすめ', data.categories[0]?.id === 'recommended'],
  ['おすすめには現行指定Skillを登録する', recommendedSkills.length === 5 && recommendedSkills.every((record) => record.kind === 'skill') && ['strict-recheck-and-refine', 'todo-add', 'エレガント・プロンプト', '戦略→実行 統括', '施策進行マネージャー'].every((name) => recommendedSkills.some((record) => record.name === name))],
  ['第2カテゴリは事業運用', data.categories[1]?.id === 'business-operations'],
  ['ADVタグのSkillは事業運用', all.filter((record) => record.projectTags.includes('ADV')).every((record) => record.category === 'business-operations')],
  ['第5カテゴリはAI・開発', data.categories[4]?.id === 'ai-development'],
  ...
  ,['カテゴリ選択後も5カテゴリの件数を算出できる', data.categories.length === 5 && Object.values(categoryCounts).every((count) => count > 0)]
  ...
  ,['エレガント・プロンプトはClaude実動未検証を記録する', recommendedSkills.some((record) => record.id === 'user.elegant-prompt' && record.environments.codex.evidenceType === 'clean_room_test' && record.environments.claude.evidenceType === 'file_hash_verified' && /実動テストは未実施/.test(record.environments.claude.verificationNote || ''))]
  ,['廃止承認済みのprompt-engineering-assistantを再表示しない', !all.some((record) => record.name === 'prompt-engineering-assistant')]
  ,['共通Skill 2件はおすすめ・AI運用・Codex実動確認済みである', ['strategy-execution-orchestrator', 'initiative-progress-manager'].every((id) => { const record=all.find((item) => item.id === `user.${id}`); return record?.category === 'recommended' && record.projectTags.includes('AI運用') && record.provider.type === 'user' && /共通正本Skill/.test(record.provider.name) && record.environments.codex.configured && record.environments.codex.evidenceType === 'clean_room_test' && record.environments.claude.configured && record.environments.claude.evidenceType === 'file_hash_verified' && /実動テストは未実施/.test(record.environments.claude.verificationNote || ''); })]
  ,['Claude中核4 Skillは配置・設定確認だけで、実動確認済みと混同しない', ['strategy-execution-orchestrator', 'initiative-progress-manager', 'elegant-prompt', 'manage-codex-claude-mirroring'].every((id) => { const record=all.find((item) => item.id === `user.${id}`); return record?.environments.claude.availability === 'installed_unverified' && record.environments.claude.evidenceType === 'file_hash_verified' && /実動テストは未実施/.test(record.environments.claude.verificationNote || ''); })]
  ,['3つの画面とフィルター順がある', ['skillsTab','integrationsTab','diagnosticsTab','integrationsScreen','diagnosticsScreen'].every((id) => html.includes(id)) && ['id="search"','id="purposeFilter"','id="projectFilter"','id="environmentControls"','data-availability="verified"','data-provider="自作"'].every((id,index,array) => !index || html.indexOf(array[index - 1]) < html.indexOf(id))]
  ...
  ,['共通Skillの表示名・依頼例・起動語が正しい', (() => { const strategy=all.find((record) => record.id === 'user.strategy-execution-orchestrator'); const progress=all.find((record) => record.id === 'user.initiative-progress-manager'); return strategy?.name === '戦略→実行 統括' && strategy.example === '施策開始！' && strategy.triggers?.includes('$strategy-execution-orchestrator') && progress?.name === '施策進行マネージャー' && progress.example === '施策進行！' && ['施策のスケ調整お願い', '$initiative-progress-manager'].every((trigger) => progress.triggers?.includes(trigger)); })()]
